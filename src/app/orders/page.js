...
import { Package, Phone, User, Calendar, CreditCard, CheckCircle2, Clock, XCircle, Star, MessageSquare, Wrench, Smartphone, ShoppingBag } from 'lucide-react';
...
function OrdersContent() {
    ...
    const { data: repairOrdersData, isLoading: isLoadingRepair, error: errorRepair, refetch: refetchRepairOrders } = useApiGet(
        ['repairOrders'],
    ...
    const normalizedRepairOrders = repairOrders.map(order => normalizeOrder(order, 'repair')).sort((a, b) => {
        return new Date(b.created_at || 0) - new Date(a.created_at || 0);
    });
    
    const normalizedPhoneOrders = phoneOrders.map(order => normalizeOrder(order, 'phone')).sort((a, b) => {
        return new Date(b.created_at || 0) - new Date(a.created_at || 0);
    });
    
    const normalizedAccessoryOrders = accessoryOrders.map(order => normalizeOrder(order, 'accessory')).sort((a, b) => {
        return new Date(b.created_at || 0) - new Date(a.created_at || 0);
    });
    ...
    const renderOrderCard = (order) => {
        ...
        if (statusLower === 'confirmed') {
            statusBadgeProps = { className: 'bg-green-500/20 text-green-300 border-green-500/30 flex items-center' };
            statusIcon = <CheckCircle2 size={12} className="mr-1" />;
        } else if (statusLower === 'pending' || statusLower === 'processing') {
            statusBadgeProps = { className: 'bg-yellow-500/20 text-yellow-300 border-yellow-500/30 flex items-center' };
            statusIcon = <Clock size={12} className="mr-1" />;
        } else if (statusLower === 'cancelled') {
            statusBadgeProps = { className: 'bg-red-500/20 text-red-300 border-red-500/30 flex items-center' };
            statusIcon = <XCircle size={12} className="mr-1" />;
        }

        const paymentStatusLower = (order.payment_status || '').toLowerCase();
        const paymentStatusDisplay = order.payment_status_display || order.payment_status || 'Unknown';
        let paymentBadgeProps = { className: 'bg-yellow-500/20 text-yellow-300 border-yellow-500/30 flex items-center' };
        let paymentIcon = <Clock size={12} className="mr-1" />;
        
        if (paymentStatusLower === 'paid') {
            paymentBadgeProps = { className: 'bg-green-500/20 text-green-300 border-green-500/30 flex items-center' };
            paymentIcon = <CheckCircle2 size={12} className="mr-1" />;
        }

        return (
            <div key={order.id || order.order_number} className="p-4 bg-white/5 rounded-lg border border-accent/10 hover:border-accent/30 transition-all relative overflow-hidden">
                ...
                <div className="absolute top-0 right-0 bg-primary/80 backdrop-blur-sm px-2 py-0.5 rounded-bl-lg flex items-center gap-1 text-xs font-medium">
                    ...
                    <span className="text-accent/80">{getOrderTypeLabel(order.orderType)}</span>
                </div>
                        ...
                        <div className="grid grid-cols-4 items-center  gap-y-1 text-sm">
                            <div className="flex items-center gap-1.5">
                                ...
                                <span className="text-xs text-accent/60">
                                    {order.orderType === 'accessory' ? 'Product' : order.orderType === 'repair' ? 'Repair Service' : 'Phone Model'}:
                                </span>
                                <span className="font-medium text-accent">{order.productName}</span>
                            </div>
                            {order.brandName && order.brandName !== 'N/A' && (
                                <div className="flex items-center gap-1.5">
                                    ...
                                    <span className="font-medium text-accent">{order.brandName}</span>
                                </div>
                            )}
                            <div className="flex items-center gap-1.5">
                                <span className="text-xs text-accent/60">Customer:</span>
                                <span className="font-medium text-accent">{order.customer_name || 'N/A'}</span>
                            </div>
                            <div className="flex items-center gap-1.5">
                                <span className="text-xs text-accent/60">Phone:</span>
                                <span className="font-medium text-accent">{order.customer_phone || 'N/A'}</span>
                            </div>
                            {order.quantity > 1 && (
                                <div className="flex items-center gap-1.5">
                                    <span className="text-xs text-accent/60">Qty:</span>
                                    <span className="font-medium text-accent">{order.quantity}</span>
                                </div>
                            )}
                        </div>
                            ...
                            <div>
                                ...
                                {order.quantity > 1 && (
                                    <p className="text-xs text-accent/60 mt-0.5">
                                        {order.quantity} × {((parseFloat(order.total_amount) || 0) / order.quantity).toFixed(2)} {order.currency || 'EUR'}
                                    </p>
                                )}
                            </div>
                            ...
                            {isLoading ? (
                                ...
                                <div className="text-center py-12">
                                    ...
                                    <p className="text-accent/80 text-lg">
                                        {activeTab === 'all' ? 'No orders found.' : `No ${activeTab === 'repair' ? 'repair service' : activeTab === 'phone' ? 'phone' : 'accessory'} orders found.`}
                                    </p>
                                    <p className="text-accent/60 text-sm mt-2">
                                        {activeTab === 'all' 
                                            ? 'Your orders will appear here once you place an order.'
                                            : `Try switching to a different tab or place a ${activeTab === 'repair' ? 'repair service' : activeTab === 'phone' ? 'phone' : 'accessory'} order.`
                                        }
                                    </p>
                                </div>
                            ) : (
                                <div className="space-y-3">
                                    {activeTab === 'all' ? (
                                        ...
                                        <>
                                            ...
                                            {normalizedRepairOrders.length > 0 && (
                                                <div className="space-y-3">
                                                    <div className="flex items-center gap-2 pb-1.5 border-b border-accent/20">
                                                        ...
                                                        <div>
                                                            <h3 className="text-base font-semibold text-secondary">Repair Services</h3>
                                                            <p className="text-xs text-accent/60">{normalizedRepairOrders.length} {normalizedRepairOrders.length === 1 ? 'order' : 'orders'}</p>
                                                        </div>
                                                    </div>
                                                    {normalizedRepairOrders.map((order) => renderOrderCard(order))}
                                                </div>
                                            )}
                                            ...
                                            <div>
                                                            <h3 className="text-base font-semibold text-secondary">New Phones</h3>
                                                            <p className="text-xs text-accent/60">{normalizedPhoneOrders.length} {normalizedPhoneOrders.length === 1 ? 'order' : 'orders'}</p>
                                                        </div>
                                                    </div>
                                                    {normalizedPhoneOrders.map((order) => renderOrderCard(order))}
                                                </div>
                                            )}
                                            ...
                                            {normalizedAccessoryOrders.length > 0 && (
                                                <div className="space-y-3">
                                                    <div className="flex items-center gap-2 pb-1.5 border-b border-accent/20">
                                                        ...
                                                        <div>
                                                            <h3 className="text-base font-semibold text-secondary">Accessories</h3>
                                                            <p className="text-xs text-accent/60">{normalizedAccessoryOrders.length} {normalizedAccessoryOrders.length === 1 ? 'order' : 'orders'}</p>
                                            </div>
                                        </div>
                                                    {normalizedAccessoryOrders.map((order) => renderOrderCard(order))}
                                                </div>
                                            )}
                                        </>
                                    ) : (
                ...
                <DialogContent className="bg-primary border-accent/20 text-white max-w-md">
                    <DialogHeader>
                        ...
                        <DialogDescription className="text-accent/80">
                            {selectedOrder && (
                                <>Review for {selectedOrder.productName || selectedOrder.phone_model_name || selectedOrder.product_title || 'this product'}</>
                            )}
                        </DialogDescription>
                    </DialogHeader>
