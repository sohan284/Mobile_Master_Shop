...
export default function PriceBreakdownPage({ params }) {
      ...
      try {
        ...
        try {
          response = await apiFetcher.post(
            "/api/repair/repair-prices/calculate_price/",
            requestBody
          );
        } catch (authError) {
          console.log(
        ...
        if (err.response?.status === 401) {
          setError(
            errorMessage ||
              "Authentication required. Please log in to calculate prices."
          );
        } else if (err.response?.status === 403) {
          setError(
            errorMessage ||
              "Access denied. You do not have permission to calculate prices."
          );
        ...
        const fallbackData = {
          phone_model:
            phoneInfo?.name ||
            `${brand.charAt(0).toUpperCase() + brand.slice(1)} Phone`,
          brand:
            phoneInfo?.brand || brand.charAt(0).toUpperCase() + brand.slice(1),
  ...
  const getAvailableTimeSlots = (dateString) => {
    ...
    // Define time ranges based on day of week
    let timeRanges = [];

    if (dayOfWeek === 1) {
      // Monday
      // Monday: 14:00-19:00 (2-7 pm)
      timeRanges = [{ start: 14 * 60, end: 19 * 60 }]; // 840-1140 minutes
    } else if (dayOfWeek >= 2 && dayOfWeek <= 6) {
      // Tuesday to Saturday
      // Tuesday to Saturday: 10:00-13:00 (10 am-1 pm) OR 14:00-19:00 (2-7 pm)
      timeRanges = [
        { start: 10 * 60, end: 13 * 60 }, // 600-780 minutes
        { start: 14 * 60, end: 19 * 60 }, // 840-1140 minutes
      ];
    } else if (dayOfWeek === 0) {
      // Sunday
      // Sunday: 10:00-13:00 (10 am-1 pm)
      timeRanges = [{ start: 10 * 60, end: 13 * 60 }]; // 600-780 minutes
    }
  ...
  const validateScheduleTime = (date, time) => {
    ...
    if (dayOfWeek === 1) {
      ...
      isValid =
        (selectedTimeInMinutes >= 600 && selectedTimeInMinutes < 780) ||
  ...
  const handleProceedToBooking = async () => {
    ...
    const timeValidationError = validateScheduleTime(
      scheduleDate,
      scheduleTime
    );
  ...
  const createOrder = async () => {
    try {
      ...
      const scheduledDateTime =
        scheduleDate && scheduleTime ? `${scheduleDate} ${scheduleTime}` : null;

      // Get customer info from user or form
      const customerName =
        user?.name || user?.username || customerInfo.username || "Customer";
      ...
      if (typeof window !== "undefined" && priceData) {
        const bookingPayload = {
          ...
          payment_intent_id:
            order?.payment?.payment_intent_id || order?.payment_intent || null,
          client_secret:
          ...
          summary: {
            ...
            priceAfterItemDiscount: parseFloat(
              priceData.price_after_item_discount || "0"
            ),
              ...
              <div className="bg-gradient-to-br from-gray-100/40 to-gray-300/40 backdrop-blur-sm rounded-xl shadow-lg border border-gray-600/20 p-5 mb-4">
                ...
                <Breadcrumb
                  items={[
                    ...
                    {
                      label: phoneInfo?.name || t("phoneModel"),
                      href: `/repair/${brand}/${phoneId}`,
                      icon: Smartphone,
                    },
                    ...
                    <div className="bg-white rounded-lg border p-3">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <h3 className="font-semibold text-base text-secondary truncate">
                            {priceData.phone_model}
                          </h3>
                    ...
                    <div className="bg-white rounded-lg border p-3">
                      ...
                      <div className="space-y-2">
                        {priceData.items.map((item, index) => (
                          ...
                          >
                            <div className="flex-1 min-w-0">
                              <h4 className="font-medium text-sm text-secondary truncate">
                                {item.problem_name}
                              </h4>
                              <p className="text-xs text-gray-600">
                                {t("partType")}:{" "}
                      ...
                      <div className="space-y-3">
                          ...
                          <div className="relative group">
                            ...
                            <Input
                              ...
                              onChange={(e) => {
                                ...
                                // Clear selected time if it's not valid for the new date
                                if (scheduleTime) {
                                  const error = validateScheduleTime(
                                    newDate,
                                    scheduleTime
                                  );
                                  if (error) {
                        ...
                        <div>
                          <Label
                            ...
                            className="text-gray-600 text-xs font-medium mb-2 flex items-center gap-2"
                          >
                            <Clock className="w-4 h-4 text-gray-400" />
                            {t("scheduleTime")} *
                          </Label>
                          {!scheduleDate ? (
                            <div className="w-full bg-gray-50 border-2 border-gray-200 rounded-lg p-4 text-center">
                            ...
                            <div>
                              <div className="grid grid-cols-6 gap-2 max-h-48 overflow-y-auto p-2 bg-gray-50 rounded-lg border border-gray-200">
                                {getAvailableTimeSlots(scheduleDate).map(
                                  (slot) => (
                                    <button
                                      key={slot}
                                      type="button"
                                      ...
                                      className={`px-3 py-2 text-xs font-medium rounded-lg border-2 transition-all duration-200 ${
                                        scheduleTime === slot
                                          ? "bg-secondary text-primary border-secondary shadow-md scale-105"
                                          : "bg-white text-secondary border-gray-300 hover:border-secondary hover:bg-secondary/10"
                                      }`}
                                    >
                                      {formatTime(slot)}
                                    </button>
                  ...
                  <div className="md:col-span-1">
                        ...
                        <div className="space-y-2 text-sm">
                          <div className="flex justify-between">
                            ...
                            <span className="font-medium text-secondary">
                              €{parseFloat(priceData.subtotal).toFixed(2)}
                            </span>
                          </div>
                          ...
                          {parseFloat(priceData.item_discount) > 0 && (
                            <div className="flex justify-between text-secondary">
                              ...
                              <span>
                                -€
                                {parseFloat(priceData.item_discount).toFixed(2)}
                              </span>
                            </div>
                          )}
                          ...
                          {parseFloat(priceData.website_discount) > 0 && (
                            <div className="flex justify-between text-secondary">
                              <span>
                                {t("websiteDiscountBreakdown")} (
                                {priceData.website_discount_percentage}%):
                              </span>
                              <span>
                                -€
                                {parseFloat(priceData.website_discount).toFixed(
                                  2
                                )}
                              </span>
                            </div>
                          )}
                          {parseFloat(priceData.website_discount_amount) >
                            0 && (
                            <div className="flex justify-between text-secondary">
                              ...
                              <span>
                                -€
                                {parseFloat(
                                  priceData.website_discount_amount
                                ).toFixed(2)}
                              </span>
                            </div>
                          )}
                          ...
                          <div className="border-t border-gray-300 pt-2 mt-2">
                            <div className="flex justify-between text-base font-bold">
                              ...
                              <span className="text-secondary">
                                €{parseFloat(priceData.total_amount).toFixed(2)}
                              </span>
                            </div>
                          </div>
                          ...
                          {parseFloat(priceData.total_discount) > 0 && (
                            <div className="text-center text-secondary font-medium text-xs pt-1">
                              You saved €
                              {parseFloat(priceData.total_discount).toFixed(2)}!
                            </div>
                          )}
                        </div>
                      </div>
                    ...
                    <div className="flex flex-col sm:flex-row gap-3 justify-center mt-10">
                      ...
                      <CustomButton
                        onClick={handleProceedToBooking}
                        disabled={
                          !scheduleDate || !scheduleTime || !!scheduleError
                        }
