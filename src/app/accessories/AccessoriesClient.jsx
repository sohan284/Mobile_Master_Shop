...
export default function AccessoriesClient({ initialAccessories = [], isLoading: initialLoading = false, error: initialError = null }) {
  ...
  useEffect(() => {
    const items = initialAccessories.length ? initialAccessories : fallbackItems;
    setRandomizedItems([...items].sort(() => Math.random() - 0.5));
  }, [initialAccessories, fallbackItems]);

  const allItems =
    randomizedItems.length > 0 ? randomizedItems : initialAccessories || fallbackItems;
