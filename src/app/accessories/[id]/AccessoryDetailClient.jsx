...
export default function AccessoryDetailClient({ 
  ...
  // Only fetch from API if we don't have initialAccessory and no sessionItem
  const { data: apiResponse, isLoading: apiLoading } = useApiGet(
    ['accessory', id],
    () => apiFetcher.get(`/api/accessories/products/${id}/`),
    { enabled: !sessionItem && !initialAccessory && !!id }
  );

  const accessory = useMemo(() => {
    return sessionItem || initialAccessory || apiResponse?.data || null;
  }, [sessionItem, initialAccessory, apiResponse?.data]);

  const isLoading = initialLoading || (apiLoading && !accessory);
