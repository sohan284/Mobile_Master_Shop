...
export default function RepairOrdersPage() {
  ...
  const updateStatusMutation = useApiPatch({
    onSuccess: (data, variables) => {
      ...
      queryClient.invalidateQueries({ queryKey: ['repairOrders', selectedStatus, currentPage] });
  ...
  const handleView = async (order) => {
    try {
      await apiFetcher.patch(`/api/repair/orders/${order.id}/`, { is_read: true });
  ...
  const { data: ordersData, isLoading: isLoadingOrders, error: errorOrders } = useApiGet(
    ['repairOrders', selectedStatus, currentPage],
    () => {
